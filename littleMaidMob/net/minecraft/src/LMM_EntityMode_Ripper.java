...
public class LMM_EntityMode_Ripper extends LMM_EntityModeBase {
	...
	public void addEntityMode(EntityAITasks pDefaultMove, EntityAITasks pDefaultTargeting) {
		...
		// ltasks[0].addTask(11, new EntityAILeapAtTarget(pentitylittlemaid,
		// 0.3F));
	...
	public boolean attackEntityAsMob(int pMode, Entity pEntity) {
		...
		if (owner.getSwingStatusDominant().canAttack()) {
			...
			if (pEntity instanceof EntityCreeper) {
				...
				// なんでPrivateにかえたし
				try {
					lis.damageItem((Integer) ModLoader.getPrivateValue(EntityCreeper.class, (EntityCreeper) pEntity, 1), owner);
	...
	public void init() {
		...
		// ModLoader.addLocalization("littleMaidMob.mode.TNT-D", "ja_JP",
		// "TNT-D");
	...
	public void onUpdate(int pMode) {
		...
		if (pMode == mmode_Detonator && owner.isEntityAlive()) {
			...
			if (timeSinceIgnited > -1) {
				...
				mod_LMM_littleMaidMob.Debug(String.format("ID:%d(%s)-dom:%d(%d)", owner.entityId, owner.worldObj.isRemote ? "C" : "W", owner.maidDominantArm,
						owner.maidInventory.currentItem));
				...
				if (owner.maidInventory.isItemExplord(owner.maidInventory.currentItem) && timeSinceIgnited++ > 30) {
					...
					// Mobによる破壊の是非
					// boolean lflag =
					// owner.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing");
					// owner.worldObj.createExplosion(owner, owner.posX,
					// owner.posY, owner.posZ, 3F, lflag);
				}
			}
		}
	}
	...
	public void updateAITick(int pMode) {
		...
		if (litemstack != null && (owner.getAttackTarget() instanceof EntityCreeper || owner.getEntityToAttack() instanceof EntityTNTPrimed)) {
