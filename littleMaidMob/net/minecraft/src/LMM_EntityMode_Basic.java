			...
			if (!lentity.isDead) {
				if (owner.getDistanceSqToEntity(lentity) < 5D) {
					...
					if (!owner.isMaidWaitEx()) {
						...
						// mod_littleMaidMob.Debug(String.format("Rerute:%b",
						// hasPath()));
						if (myChest != null) {
							myChest.closeChest();
							myChest = null;
	...
	public void putChest() {
		...
		if (owner.getSwingStatusDominant().canAttack() && myChest != null) {
			...
			if (is != null
					...
					// ((ItemArmor)is.getItem()).armorType == 0)
					)) {
				...
				for (int j = 0; j < myChest.getSizeInventory() && is.stackSize > 0; j++) {
					...
					if (isc == null) {
						// mod_littleMaidMob.Debug(String.format("%s -> NULL",
						// is.getItemName()));
						...
						// mod_littleMaidMob.Debug(String.format("%s -> %s",
						// is.getItemName(), isc.getItemName()));
						f = true;
			...
			// mod_littleMaidMob.Debug(String.format("getchest3:%d",
			// maidSearchCount));
